...
contract ShidenPass is ERC721Enumerable, Ownable {
    ...
    /// @param user caller's acount
    /// @return passHolderStatus boolean
    function isPassHolder(address user) private view returns (bool) {
        // The isRegistered() call returns true if the account is registerd in AstarBase.
        // In this contract this call can be omitted since the below call to checkStakerStatus() will
        // return 0 if the account is not registered.
        // This code is left as an example of possible use of AstarBase
        bool registeredInAstarBase = ASTARBASE.isRegistered(user);
        if (!registeredInAstarBase) return false;
    ...
    function setAstarBaseAddress(address astarBaseDeployedAddress) public {
        ASTARBASE = AstarBase(astarBaseDeployedAddress);
    }
